...
int main()
{
    ...
    #ifdef debug
        std::cout << "Tamaño de int: " << sizeof(intVar) << " bytes" << std::endl;
        std::cout << "Tamaño de double: " << sizeof(doubleVar) << " bytes" << std::endl;
    ...
    int k = h++; // Post-incremento: k se asigna el valor de h, luego h se incrementa;k=3->h=4
    
    //++h -> h=h+1;
    std::cout << "Valor de k después de post-incremento: " << k << std::endl;

    int j = ++h; // Pre-incremento: h se incrementa, luego j se asigna el valor de h; h=5, j=5
    std::cout << "Valor de j después de pre-incremento: " << j << std::endl;

    // Operadores de asignación compuesta
    h += 8; // Equivalente a h = h + 8; h=13
    std::cout << "Valor de h después de h += 8: " << h << std::endl;

    h *= 2; // Equivalente a h = h * 2;h=26
    std::cout << "Valor de h después de h *= 2: " << h << std::endl;

    h = h * 2; // Multiplicación;h=52
